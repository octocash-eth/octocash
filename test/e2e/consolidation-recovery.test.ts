...
describe("Scenario 8: Browser Recovery", () => {
  ...
  test("browser close mid-execution - state is persisted and recoverable", async () => {
    const sourceTokens: TokenAmount[] = [
      makeToken(ETH_ADDRESS, 100000000000000000n, 137, WALLET, "ETH", 18), // 0.1 ETH on Polygon
    ];
  ...
  test("recover from paused state - failed step can be retried", async () => {
    const sourceTokens: TokenAmount[] = [
      makeToken(USDC_ADDRESS, 1000000n, 10, WALLET), // 1 USDC on Optimism
    ];
    ...
    const state: ConsolidationState = {
      ...
      results: {
        [plan[0].id]: {
          ...
          status: "success",
          chainId: plan[0].chainId,
          actualOutput: makeToken(USDC_ADDRESS, 798450000n, 137, WALLET),
