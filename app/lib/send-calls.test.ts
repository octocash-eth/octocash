...
describe("prepareSendCalls", () => {
  ...
  test("throws error when transaction reverted", async () => {
    const client = {
      switchChain: vi.fn().mockResolvedValue(undefined),
      addChain: vi.fn().mockResolvedValue(undefined),
      sendCalls: vi.fn().mockResolvedValue({ id: "test-id" }),
    ...
    await expect(prepareSendCalls(client)("test", 1, "0x0000000000000000000000000000000000000000", [])).rejects.toThrow(
