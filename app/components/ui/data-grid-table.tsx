...
const headerCellSpacingVariants = cva("px-2.5 h-9 md:px-4");

const bodyCellSpacingVariants = cva("px-2.5 py-2 md:px-4 md:py-3");
  ...
  return (
    <tr
      ...
      className={cn(
        ...
        props.tableLayout?.striped && "bg-transparent",
  ...
  return (
    <tr
      className={cn(
        ...
        props.tableLayout?.cellBorder && "[&_>:last-child]:border-e-0",
        props.tableLayout?.striped && "odd:bg-muted/90 hover:bg-transparent odd:hover:bg-muted",
...
function DataGridTableBodyRowCell<TData>({ children, cell }: { children: ReactNode; cell: Cell<TData, unknown> }) {
  const { props } = useDataGrid();
...
function DataGridTable<TData>() {
  ...
  return (
    <DataGridTableBase>
      ...
      {(props.tableLayout?.striped || !props.tableLayout?.rowBorder) && <DataGridTableRowSpacer />}

      <DataGridTableBody>
        {isLoading && props.loadingMode === "skeleton" && pagination?.pageSize ? (
          // Show skeleton loading immediately
          Array.from({ length: pagination.pageSize }).map((_, rowIndex) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: Skeleton rows are temporary placeholders without real IDs
